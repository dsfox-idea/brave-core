...
class SettingsSystemPageElement extends SettingsSystemPageElementChromium {
  ...
  // eagerly: a plain field initializer would run (and CHECK-fail) on every
  // platform. A getter keeps it lazy, only evaluated from within the
  // isMac_-gated branch of the mangled template.
  override get warnBeforeQuittingLabel_() {
    return loadTimeData.getString('warnBeforeQuitting')
  }
}
