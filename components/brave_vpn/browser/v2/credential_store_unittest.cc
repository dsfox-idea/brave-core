...
TEST_F(CredentialStoreTest, SubscriberCredentialRoundTrip) {
  ...
  const std::optional<Credential> credential =
      store_.GetValidSubscriberCredential();
  ASSERT_TRUE(credential.has_value());
  EXPECT_EQ(credential->value, kTestCredential);
  EXPECT_EQ(credential->expiration, Future());
}
...
TEST_F(CredentialStoreTest, SkusCredentialRoundTripAndLastExpiryStamp) {
  ...
  const std::optional<Credential> credential = store_.GetValidSkusCredential();
  ASSERT_TRUE(credential.has_value());
  EXPECT_EQ(credential->value, kTestSkusCredential);
...
TEST_F(CredentialStoreTest, ValidCredentialCarriesExpiration) {
  ...
  store_.SetSkusCredential(
      {.value = kTestSkusCredential, .expiration = Future()});
  const std::optional<Credential> skus = store_.GetValidSkusCredential();
  ASSERT_TRUE(skus.has_value());
  EXPECT_EQ(skus->expiration, Future());

  store_.SetSubscriberCredential(
      {.value = kTestCredential, .expiration = Future()});
  const std::optional<Credential> subscriber =
      store_.GetValidSubscriberCredential();
  ASSERT_TRUE(subscriber.has_value());
  EXPECT_EQ(subscriber->expiration, Future());
}
