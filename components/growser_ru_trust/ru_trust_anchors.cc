...
base::ListValue GetTrustAnchorsPrefDefault(
  ...
  base::DictValue constraints;
  constraints.Set("permitted_dns_names", std::move(permitted_dns_names));

  base::DictValue anchor;
  anchor.Set("certificate", kRussianTrustedRootCaBase64);
  anchor.Set("constraints", std::move(constraints));

  base::ListValue anchors;
