...
TEST_F(RemoteModelsFetcherTest, InvalidJSON) {
  // SimulateFetch()'s default empty response body reproduces
  // APIRequestHelper leaving value_body() as a default (NONE-type) Value,
  // which happens when the response body fails to parse as JSON while still
  // reporting a 2XX response code.
  SimulateFetch();
