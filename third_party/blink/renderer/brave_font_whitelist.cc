...
// Growser-205: the fallback for every Linux that is not one of the
// three distros above.
//
// Upstream leaves this as the EMPTY set, and empty is not "no
// preference" here - AllowFontByFamilyName returns true for every
// font when the list is empty, so the farbling path is never reached
// and there is no protection at all on any other distro. Brave say so
// themselves: they do not apply these protections on Linux "because of
// difficulties in determining which fonts are OS fonts for each
// distro" (brave-browser#34169 added the three; nothing followed).
//
// These 44 are what all three of those lists have in common, which is
// the part that does not depend on the distro - DejaVu, Liberation,
// Noto, the Lohit families. Generated by
// scripts/linux/make-font-fallback.py, which also has a --check mode.
//
// The intersection rather than the union on purpose: over-claiming
// costs real rendering (brave-browser#55219 is sites breaking on
// Fedora KDE, where a list IS applied), while under-claiming only
// sends a font through farbling instead of waving it past.
constexpr auto kFontWhitelist =
    base::MakeFixedFlatSet<std::string_view>({
        "abyssinica sil",
        "c059",
