...
// Regression test for the bug this delegate change fixes: detaching a parent
// tab together with its child into a brand new window (e.g. "Move tabs to
// new window") used to flatten the child out to a top-level tab in the new
// window, because TabStripModel::DetachTabsAndCollectionsForInsertion() only
// special-cased whole groups/splits, not tree-tab subtrees. The child must
// stay nested under the parent in the destination window, and both the
// source and destination TreeTabModel must reflect the move.
//
// This drives TabStripModel::DetachTabsAndCollectionsForInsertion() and
// InsertDetachedTreeTabNodeAt() directly - the same primitives
// chrome::MoveTabsToNewWindow() and drag-to-new-window both funnel through -
// rather than the full command, to isolate the tree hierarchy behavior from
// unrelated window/view plumbing.
IN_PROC_BROWSER_TEST_F(TreeTabsBrowserTest,
